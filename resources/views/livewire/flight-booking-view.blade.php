<div class="container mx-auto mt-8 p-4">
    <h3 class="text-center text-2xl font-bold mb-6">Your Booked Flights</h3>
    <div class="overflow-x-auto shadow-lg rounded-lg bg-white p-4">
        <table class="min-w-full table-auto">
            <thead>
                <tr class="bg-gray-100">
                    <th class="px-4 py-2 text-sm font-medium text-gray-700 text-center">Flight Name</th>
            ...
            <tbody>
                @foreach($flights as $flight)
                    <tr class="border-t border-gray-200">
                        <td class="px-4 py-2 text-sm text-gray-700 text-center">{{ $flight->flight_name }}</td>
                        ...
                        <td class="px-4 py-2 text-sm text-gray-700 text-center">
                            <button wire:click="edit({{ $flight->id }})" class="inline-block bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Edit</button>
                            <button wire:click="delete({{ $flight->id }})" class="inline-block bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 ml-2">Delete</button>
                            <button onclick="window.location.href='{{ route('flights.payment', $flight->id) }}'" class="inline-block bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                Proceed To Payment
                            </button>
                        </td>
                    </tr>
